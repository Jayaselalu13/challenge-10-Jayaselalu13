...
function HomePage() {
    ...
    const [mostLikedPosts, setMostLikedPosts] = (0, react_1.useState)([]);
    const [currentPage, setCurrentPage] = (0, react_1.useState)(1);
    const [totalPages, setTotalPages] = (0, react_1.useState)(3);
    ...
    const fetchPosts = () => __awaiter(this, void 0, void 0, function* () {
        try {
            setLoading(true);
            const fetchedPosts = yield api_1.blogApi.getPosts(currentPage, 10);
    ...
    const fetchMostLikedPosts = () => __awaiter(this, void 0, void 0, function* () {
        try {
            const fetchedMostLiked = yield api_1.blogApi.getMostLikedPosts(1, 3);
    ...
    const handleLike = (postId) => __awaiter(this, void 0, void 0, function* () {
        try {
            yield api_1.blogApi.likePost(postId);
            // Update posts state
            ...
            setMostLikedPosts(mostLikedPosts.map((post) => post.id === postId
                ? Object.assign(Object.assign({}, post), { isLiked: !post.isLiked, likes: post.isLiked ? post.likes - 1 : post.likes + 1 }) : post));
        }
        catch (error) {
    ...
    if (loading) {
        return (<div className="w-full relative bg-white min-h-screen">
        <header_1.default />
        <div className="animate-pulse max-w-[1200px] mx-auto px-4 py-8">
          {Array.from({ length: 3 }).map((_, i) => (<div key={i} className="bg-white rounded-lg p-6 shadow-sm mb-6">
    ...
    return (<>
        ...
        <div className="absolute top-[128px] left-1/2 transform -translate-x-1/2 w-full max-w-[1200px] px-4 lg:px-0 flex flex-col items-start justify-start text-[#374151] pb-24">
          <div className="self-stretch flex flex-col lg:flex-row items-start justify-start gap-6 lg:gap-12">
            ...
            <div className="w-full lg:w-[807px] flex flex-col items-start justify-start gap-6">
              ...
              {/* Blog Posts */}
              {posts.map((post, index) => (<div key={`main-${post.id}`} className="w-full">
                  <div className="w-full bg-white flex flex-row items-start justify-start gap-6">
                    <link_1.default href={`/post/${post.id}`}>
                      <img className="w-[340px] relative rounded-md h-[258px] object-cover hover:opacity-90 transition-opacity cursor-pointer" alt="" src={getImageUrl(post.imageUrl)}/>
                    </link_1.default>
                    <div className="flex-1 flex flex-col items-start justify-start gap-4">
                      <div className="self-stretch flex flex-col items-start justify-start gap-3">
                        ...
                        <div className="flex flex-row items-start justify-start gap-2 flex-wrap">
                          {post.tags.map((tag, tagIndex) => (<div key={`main-${post.id}-tag-${tagIndex}`} className="rounded-lg bg-white border-[#D5D7DA] border-solid border-[1px] box-border h-7 flex flex-row items-center justify-center p-2">
                      ...
                      <div className="self-stretch flex flex-row items-center justify-start gap-3">
                        <div className="flex flex-row items-center justify-start gap-2">
                          <img className="w-10 relative rounded-[50%] max-h-full object-cover" alt="" src={getAvatarUrl(post.author.avatarUrl)}/>
                ...
                year: 'numeric'
                      ...
                      <div className="flex flex-row items-center justify-start gap-5 text-[#535862]">
                        <button onClick={() => handleLike(post.id)} className="flex flex-row items-center justify-start gap-1.5 hover:text-red-500 transition-colors">
                  ...
                  {index < posts.length - 1 && (<div className="w-full relative border-[#E5E7EB] border-solid border-t-[1px] box-border h-px my-6"/>)}
                </div>))}

              {/* Pagination */}
              <div className="self-stretch flex flex-row items-center justify-center gap-2">
                <button onClick={() => handlePageChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="px-3 py-2 flex flex-row items-center justify-center gap-1 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-[#374151]">
                  <lucide_react_1.ChevronLeft className="w-4 h-4"/>
                  <span className="text-sm font-medium">Previous</span>
                </button>
                {Array.from({ length: totalPages }).map((_, i) => (<button key={i + 1} onClick={() => handlePageChange(i + 1)} className={`w-10 h-10 rounded-full box-border flex flex-row items-center justify-center text-sm font-medium transition-colors ${currentPage === i + 1
                ...
                <button onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="px-3 py-2 flex flex-row items-center justify-center gap-1 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-[#374151]">
            ...
            <div className="w-full lg:w-[357px] flex flex-col items-start justify-start gap-8">
              <div className="self-stretch flex flex-col items-start justify-start gap-6">
                ...
                <div className="self-stretch flex flex-col items-start justify-start gap-6">
                  {mostLikedPosts.map((post, index) => (<div key={`liked-${post.id}`}>
                      <div className="self-stretch flex flex-col items-start justify-start gap-3">
                        <link_1.default href={`/post/${post.id}`}>
                        ...
                        <div className="self-stretch flex flex-row items-center justify-start gap-3">
                          <div className="flex flex-row items-center justify-start gap-2">
                            <img className="w-6 relative rounded-[50%] h-6 object-cover" alt="" src={getAvatarUrl(post.author.avatarUrl)}/>
                ...
                year: 'numeric'
                        ...
                        <div className="flex flex-row items-center justify-start gap-4 text-xs text-[#9CA3AF]">
                          <button onClick={() => handleLike(post.id)} className="flex flex-row items-center justify-start gap-1 hover:text-red-500 transition-colors">
                            <lucide_react_1.ThumbsUp className={`w-4 h-4 ${post.isLiked ? "fill-red-500 text-red-500" : ""}`}/>
                      ...
                      {index < mostLikedPosts.length - 1 && (<div className="self-stretch relative border-[#E5E7EB] border-solid border-t-[1px] box-border h-px my-6"/>)}
                    </div>))}
                </div>
              </div>
            </div>
      ...
      <div className="block lg:hidden w-full relative bg-white min-h-screen text-left text-sm text-[#9CA3AF] font-inter">
        ...
        <div className="w-full px-4 py-6 flex flex-col items-start justify-start gap-6 text-[#374151] mt-4 mb-20">
          ...
          <div className="self-stretch flex flex-col items-start justify-start gap-6">
            {posts.map((post, index) => (<div key={`mobile-${post.id}`} className="w-full max-w-full bg-white flex flex-col items-start justify-start gap-3 overflow-hidden">
                <div className="w-full flex flex-col items-start justify-start gap-2 overflow-hidden">
                  <link_1.default href={`/post/${post.id}`} className="w-full">
                  ...
                  <div className="w-full flex flex-row items-start justify-start gap-2 flex-wrap">
                    {post.tags.map((tag) => (<div key={tag} className="rounded-lg bg-white border-[#D5D7DA] border-solid border-[1px] box-border h-6 flex flex-row items-center justify-center px-2 flex-shrink-0">
                ...
                <div className="flex flex-row items-center justify-start gap-4 text-xs text-[#535862]">
                  <button onClick={() => handleLike(post.id)} className="flex flex-row items-center justify-start gap-1 hover:text-red-500 transition-colors">
                    <lucide_react_1.ThumbsUp className={`w-5 h-5 ${post.isLiked ? "fill-red-500 text-red-500" : ""}`}/>
          ...
          {/* Mobile Pagination */}
          <div className="self-stretch flex flex-row items-center justify-center gap-2">
            <button onClick={() => handlePageChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="w-8 h-8 rounded-full flex flex-row items-center justify-center hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-[#374151]">
              <lucide_react_1.ChevronLeft className="w-4 h-4"/>
            </button>
            {Array.from({ length: totalPages }).map((_, i) => (<button key={i + 1} onClick={() => handlePageChange(i + 1)} className={`w-8 h-8 rounded-full box-border flex flex-row items-center justify-center text-xs font-medium transition-colors ${currentPage === i + 1
                ? "bg-[#0093DD] text-white"
                : "border-[#E5E7EB] border-solid border-[1px] text-[#374151] hover:bg-gray-50"}`}>
                {i + 1}
              </button>))}
            <button onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="w-8 h-8 rounded-full flex flex-row items-center justify-center hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-[#374151]">
              <lucide_react_1.ChevronRight className="w-4 h-4"/>
            </button>
          </div>
          ...
          <div className="self-stretch flex flex-col items-start justify-start gap-6">
            ...
            <div className="self-stretch flex flex-col items-start justify-start gap-6">
              {mostLikedPosts.map((post, index) => (<div key={`mobile-liked-${post.id}`}>
                  <div className="self-stretch flex flex-col items-start justify-start gap-3">
                    <link_1.default href={`/post/${post.id}`}>
                    ...
                    <div className="self-stretch flex flex-row items-center justify-start gap-3">
                      <div className="flex flex-row items-center justify-start gap-2">
                        <img className="w-[30px] relative rounded-[50%] h-[30px] object-cover" alt="" src={getAvatarUrl(post.author.avatarUrl)}/>
